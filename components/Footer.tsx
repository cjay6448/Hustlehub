    ...
    <>
      ...
      <footer style={{ background: C.charcoal, padding: "clamp(36px,5vw,56px) 24px 28px" }}>
        <div style={{ maxWidth: 1060, margin: "0 auto" }}>
          <div className="footer-grid">
            ...
            <div className="footer-brand">
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                <div style={{ width: 36, height: 36, borderRadius: 9, background: C.forest, display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <p style={{ fontFamily: FB, fontSize: 13, color: "rgba(255,255,255,0.48)", lineHeight: 1.72, maxWidth: 260, marginBottom: 20 }}>
              ...
              <form onSubmit={e => e.preventDefault()} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <input type="email" placeholder="your@email.ca"
                  style={{ flex: "1 1 140px", fontFamily: FB, fontSize: 13, padding: "10px 14px", borderRadius: 9, border: "none", background: "rgba(255,255,255,0.1)", color: C.white, outline: "none", minHeight: 40 }} />
                <button type="submit" style={{ fontFamily: FB, fontSize: 13, fontWeight: 700, color: C.white, background: C.forest, border: "none", padding: "10px 16px", borderRadius: 9, cursor: "pointer", minHeight: 40 }}>Join</button>
              </form>
            </div>

            {Object.entries(nav).map(([title, links]) => (
              <div key={title}>
                <h4 style={{ fontFamily: FB, fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.4)", letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 16 }}>{title}</h4>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {links.map(({ label, href }) => (
                    <Link key={label} href={href}
                      style={{ fontFamily: FB, fontSize: 13, color: "rgba(255,255,255,0.52)", textDecoration: "none" }}
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 22, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            ...
            <p style={{ fontFamily: FB, fontSize: 12, color: "rgba(255,255,255,0.28)", margin: 0 }}>Abbotsford, BC 🇨🇦</p>
          </div>
        </div>
      </footer>
    </>
